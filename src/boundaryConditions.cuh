...
void applyInflow(
    ...
    d.g[5 * PLANE + global3(x, y, 1)] = VelocitySet::G<5>::wg * d.phi[idx3_bnd] * (1.0f + 4.0f * uz);
}
...
void applyOutflow(
    ...
    const idx_t idx3_zm1 = global3(x, y, mesh::nz - 2);

    d.phi[global3(x, y, mesh::nz - 1)] = d.phi[idx3_zm1];
    ...
    constexpr_for<0, FLINKS>([&] __device__ (auto I) {
        ...
        if constexpr (VelocitySet::F<Q>::cz == -1) { 
            ...
            idx_t fluidNode = global3(xx, yy, mesh::nz - 2);
    ...
    d.g[6 * PLANE + idx3_zm1] = VelocitySet::G<6>::wg * d.phi[idx3_zm1] * (1.0f - 4.0f * physics::u_ref);
}
...
void periodicX(
    ...
    if (y <= 0 || y >= mesh::ny - 1 || z <= 0 || z >= mesh::nz - 1) return;

    const idx_t bL = global3(1, y, z);
    const idx_t bR = global3(mesh::nx - 2, y, z);
    ...
    d.phi[global3(mesh::nx - 1, y, z)] = d.phi[bL];
}
...
void periodicY(
    ...
    if (x <= 0 || x >= mesh::nx - 1 || z <= 0 || z >= mesh::nz - 1) return;

    const idx_t bB = global3(x, 1, z);
    const idx_t bT = global3(x, mesh::ny - 2, z);
    ...
    d.phi[global3(x, mesh::ny - 1, z)] = d.phi[bB];
}
