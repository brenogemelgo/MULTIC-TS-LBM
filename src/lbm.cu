...
namespace lbm
{
    ...
    __global__ void streamCollide(LBMFields d)
    {
        ...
        device::constexpr_for<0, phase::velocitySet::Q()>(
            [&](const auto Q)
            {
                const scalar_t geq = phase::velocitySet::template g_eq<Q>(phi, ux, uy, uz);
                const scalar_t hi = phase::velocitySet::template anti_diffusion<Q>(sharp, normx, normy, normz);
